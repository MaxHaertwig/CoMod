...
export class Client {
  ...
  handleRequest(request: CollaborationRequest): boolean {
    const isConnected = this.state == ClientState.Connected;
    switch (this.state) {
    ...
    return !isConnected && this.state == ClientState.Connected;
  }
